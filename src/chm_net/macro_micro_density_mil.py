...
class EnhancedConfig:
    """Configuration for CHM-Net. Supply dataset paths and columns via CLI."""
    ...
    NUM_WORKERS = 0
    PIN_MEMORY = True
    OUTPUT_DIR = "MacroMicro_MIL_Output"
...
def split_labeled_patients(valid_patients, df, config):
    labels_by_pid = {}
    labeled_patients = []
    ...
    # Requirement: 7:3 split, with validation and test merged into the same 30%.
    # The same held-out set is used for early stopping and final reporting.
    val_ids = list(val_test_ids)
    test_ids = list(val_test_ids)
    return train_ids, val_ids, test_ids, labels_by_pid
...
def train_macro_micro_classifier(config, detector, df, patient_ids_with_labels):
    print("\n" + "=" * 60)
    print("Stage 2: macro-to-micro tri-planar MIL classifier")
    print("=" * 60)

    train_ids, val_ids, test_ids, labels_by_pid = split_labeled_patients(
        patient_ids_with_labels,
        df,
        config,
    )
    print(
        f"Classifier split: train={len(train_ids)} "
        f"({config.TRAIN_RATIO:.0%}), val/test merged={len(val_ids)} "
        f"({1.0 - config.TRAIN_RATIO:.0%})"
    )
    ...
    print(f"Val/Test merged class distribution: {np.bincount(val_labels, minlength=2).tolist()}")
...
def main():
    ...
    print("=" * 80)
    print("Macro-to-Micro Density-Aware MIL Network")
    ...
    classifier, classifier_history, test_metrics = train_macro_micro_classifier(
        config,
        detector,
        df,
        labeled_patients,
    ...
    result = {
        ...
        "model_architectures": {
            "detector": {
                "type": "EnhancedUnsupervisedTransformerDetector",
                "input_size": config.DETECTOR_IMG_SIZE,
                "embed_dim": config.EMBED_DIM,
                "num_heads": config.NUM_HEADS,
                "depth": config.DEPTH,
                "parameters": detector_history.get("parameters", 0),
                "training_time_seconds": detector_time,
            },
            "classifier": {
                "type": "MacroToMicroDensityNet",
                "num_rois": config.NUM_ROIS,
                "roi_size": config.ROI_SIZE,
                "micro_encoder": "TriPlanarMicroEncoder",
                "mil": "AttentionMIL",
                "parameters": classifier_history.get("parameters", 0),
                "training_time_seconds": classifier_time,
            },
        },
        "training_parameters": {
            "detector": {
                "batch_size": config.DETECTOR_BATCH_SIZE,
                "learning_rate": config.DETECTOR_LR,
            ...
            "classifier": {
                ...
                "finetune_detector_lr": config.FINETUNE_DETECTOR_LR,
                "epochs": config.CLASSIFIER_EPOCHS,
                "freeze_detector_epochs": config.FREEZE_DETECTOR_EPOCHS,
                "sparsity_weight": config.SPARSITY_WEIGHT,
                "smoothness_weight": config.SMOOTHNESS_WEIGHT,
            },
        },
        "detector_results": {
            "best_loss": detector_history.get("best_loss", None),
            "best_epoch": detector_history.get("best_epoch", None),
        },
        "classifier_results": {
            "best_val_accuracy": classifier_history.get("best_val_acc", 0),
            "best_val_auc": classifier_history.get("best_val_auc", 0),
            "best_val_f1": classifier_history.get("best_val_f1", 0),
            "best_val_balanced_acc": classifier_history.get("best_val_balanced_acc", 0),
            "best_epoch": classifier_history.get("best_epoch", 0),
        },
        "test_results": test_metrics,
        "training_times": {
            "detector_training_seconds": detector_time,
            "classifier_training_seconds": classifier_time,
