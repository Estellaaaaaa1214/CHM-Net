...
try:
    from medmnist import AdrenalMNIST3D, INFO
except Exception:
    AdrenalMNIST3D = None
    INFO = {}
...
class Adrenal3DDataset(Dataset):
    def __init__(
        self,
        split,
        root,
    ...
    def _augment(self, x):
        ...
        scale = random.uniform(0.0, 1.0) if self.shape_intensity_reg else random.uniform(0.75, 1.25)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Macro-to-micro MIL network on AdrenalMNIST3D."
    )
    parser.add_argument("--root", default="data/public/AdrenalMNIST3D")
    parser.add_argument("--output-dir", default="./Adrenal_MacroMicro_Output")
    parser.add_argument("--size", type=int, default=64)
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--num-workers", type=int, default=4)
    parser.add_argument("--no-shape-intensity-reg", action="store_true")
...
def main():
    ...
    args.exp_dir = ensure_dir(os.path.join(args.output_dir, f"adrenal_macro_micro_{stamp}"))
    ...
    info = INFO.get("adrenalmnist3d", {})
    print("=" * 70)
    print("AdrenalMNIST3D Macro-to-Micro Density-Aware MIL Benchmark")
    print("=" * 70)
    print("Root:", args.root)
    print("Output:", args.exp_dir)
    print("Device:", device)
    print("Task:", info.get("task", "binary-class"))
    print("Labels:", info.get("label", {"0": "normal", "1": "adrenal mass"}))

    train_set = Adrenal3DDataset(
        "train",
        args.root,
        args.size,
        augment=True,
        shape_intensity_reg=not args.no_shape_intensity_reg,
        noise_std=args.noise_std,
    )
    val_set = Adrenal3DDataset(
        "val",
        args.root,
        args.size,
        augment=False,
